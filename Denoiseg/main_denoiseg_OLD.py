...
cparser.add_argument('--warmup', action='store', default=80, type=int, help='warmup epochs, no stop is allowed')
...
if __name__== '__main__':
    ...
    def criterio(out, x, scribble, mask):
        return compute_denoiseg_losses(out, x, scribble, mask,config,criterio_seg,criterio_rec)
    ...
    for batch, data in enumerate(dataloader_eval):
        ...
        model.eval()
        out = model(Xinput)
        output = get_denoiseg_outputs(out, config)
