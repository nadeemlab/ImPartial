...
cparser.add_argument('--nfore', action='store', default=1, type=int, help='nchannels foreground')
cparser.add_argument('--nback', action='store', default=2, type=int, help='nchannels background')
...
if __name__== '__main__':
    ...
    #         ix_file += 1
    #         print('Segmentation :')
    #         pd_saves_out.append([pd_files.iloc[ix_file]['prefix'], file_output_save, batch, i]) #save pandas reference
    #
    #         ix_label = 0
    #         for class_tasks_key in config.classification_tasks.keys():
    #             classification_tasks = config.classification_tasks[class_tasks_key]
    ...
    #         Y_fore_save = []
    #         Y_back_save = []
    #         #todo!: consider adding the mean rec channels
    #         for segclass in config.segclasses_channels.keys():
    #             Ylabel = Ylabels[i, int(segclass), ...].astype('int')
    #             Ypred_fore = np.sum(output['segclass_' + segclass]['fore'][i, :, ...], axis=0)
    #             Ypred_back = np.sum(output['segclass_' + segclass]['back'][i, :, ...], axis=0)
    #
    #             Y_fore_save.append(output['segclass_' + segclass]['fore'][i, :, ...])
    #             Y_back_save.append(output['segclass_' + segclass]['back'][i, :, ...])
    #
    #             for th in th_list:
    #                 rows = [_ for _ in pd_files.iloc[ix_file].values]
    #                 rows.append(segclass)
    #                 rows.append(th)
    #                 metrics = get_performance(Ylabel, Ypred_fore, threshold=th)
    #                 for key in metrics.keys():
    #                     rows.append(metrics[key])
    ...
    #                 # Y_rec_save.append(output['rec'][ch][i, 0, ...])
    #
    #             save_output_dic = config.basedir + config.model_name + '/output_images/'
    #             mkdir(save_output_dic)
    #             np.savez(save_output_dic + pd_files.iloc[ix_file]['prefix'] + '_out.npz',
    #                      fore=Y_fore_save, back=Y_back_save)
