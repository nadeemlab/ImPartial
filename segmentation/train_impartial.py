...
parser.add_argument('--exp_id', action='store', default="exp_id", type=str, help='mode')
...
print("dataset name : ", parser.dataset)
print("model_name  : ", parser.model_name)
...
# 2. Optimizer
optimizer = torch.optim.Adam(model.parameters(), parser.lr)
...
def train(model, criterio, optimizer, dataloader_train, dataloader_val, dataloader_test):
    ...
    # model_fname = "TN_unet_supervised_vectra2ch_model_eroded_labels_focal_loss_transform.pt"
    model_fname = parser.model_name + '.pt'
...
def test(model, dataloader_test, epoch, save=False):
    ...
    for batch_id, data in enumerate(dataloader_test):
        ...
        if save == True:
            ...
            save_pred_dir = '/nadeem_lab/Gunjan/code/impartial-lite/ImPartial/segmentation/predictions_focal_loss_transform/' 
...
# # #load model
if mode == 'test':
    model_file_path = '/nadeem_lab/Gunjan/code/impartial-lite/ImPartial/segmentation/TN_unet_supervised_vectra2ch_model_eroded_labels_focal_loss_transform.pt'
