...
def recseg_checkpoint_ensemble_trainer(dataloader_train,dataloader_val,model,optimizer,criterio,config):
    ...
    stopper = early_stopping(config.patience, 0, np.infty) #only to save global best model
    # stopper_train = early_stopping(config.patience, 0, np.infty)
    stopper_val = early_stopping(config.patience, 0, np.infty)
    ...
    val_model_saves_list =  config.val_model_saves_list
    val_epochs_saves_list = [0]
    val_loss_saves_list = [0]

    # train_model_saves_list = config.train_model_saves_list
    # train_epochs_saves_list = [0]
    # train_loss_saves_list = [0]

    if (len(val_model_saves_list) < nsaves):
    ...
    for epoch in range(config.EPOCHS): #config.epochs should be set super large is not supposed to be the stopping criteria
        ...
        # save_train, _ = stopper_train.evaluate(history['loss_mbatch_train'][-1])
        save_val, stop_val = stopper_val.evaluate(history['loss_val'][-1])

        # tic_list.append(time.perf_counter())
        # print( 'epoch stopper : ',tic_list[-1] - tic_list[-2])

        if save: #global best val model
        ...
        # if save_train:
        #     model_params_save(config.basedir + config.model_name + '/' + train_model_saves_list[patch_sampler],model, optimizer)  # save best model
        #     train_epochs_saves_list[-1] = epoch
        #     train_loss_saves_list[-1] = history['loss_mbatch_train'][patch_sampler]

        if save_val:
            model_params_save(config.basedir + config.model_name + '/' + val_model_saves_list[patch_sampler],model, optimizer)  # save best model
            val_epochs_saves_list[-1] = epoch
            val_loss_saves_list[-1] = history['loss_val'][-1]
            print('saving best model, epoch: ', epoch, ' to : ',val_model_saves_list[patch_sampler])

        # tic_list.append(time.perf_counter())
        # print( 'epoch saves : ',tic_list[-1] - tic_list[-2])

        string_print = 'epoch : ' + str(epoch) + ' patch_it : ' + str(patch_sampler)
        for key in tags_plot:
            string_print = string_print + ' | ' + key + ' tr, val : ' + str(
                np.round(history[key + '_mbatch_train'][-1], 3)) + ','
            string_print = string_print + str(np.round(history[key + '_val'][-1], 3))
        string_print = string_print + ' | stop_pat :' + str(stopper.counter) + ', val_stop : ' + str(stopper_val.counter)
        print(string_print)
        # print(val_epochs_saves_list)
        print('loss of validation checkpoints : ',val_loss_saves_list)
        print()

        patch_epoch_counter += 1
        if (stop_val & (patch_epoch_counter>=config.nepochs_sample_patches)) :

            print('sampling new patches...')
            patch_sampler += 1 #counter of patches iterations
            patch_epoch_counter = 0 #counter of epochs iteration for the current patches

            if (((patch_sampler >= nsaves) & (epoch > config.warmup_epochs))) :
                break
            ...
            val_epochs_saves_list.append(0)
            val_loss_saves_list.append(0)

            # train_model_saves_list.append('model_train_best_save' + str(patch_sampler) + '.pth')
            # train_epochs_saves_list.append(0)
            # train_loss_saves_list.append(0)
    ...
    history['val_loss_saves_list'] = val_loss_saves_list
    # history['train_loss_saves_list'] = train_loss_saves_list

    print('Training Ended, loading best model : ', config.basedir + config.model_name + '/' + val_model_saves_list[best_model_of_all])
    model_params_load(config.basedir + config.model_name + '/' + val_model_saves_list[best_model_of_all], model, optimizer, config.DEVICE)
    model_params_save(config.basedir + config.model_name + '/' + config.best_model, model, optimizer)  # save best model
